...
lib.callback.register('wasabi_fishing:checkRod', function(itemName, model)
...
RegisterNetEvent('wasabi_fishing:rodBroke', function(itemName)
    local fishingRod = nil

    -- Find the fishing rod based on the chosen item
    for i = 1, #Config.fishingRod do
        if Config.fishingRod[i].itemName == itemName then
            fishingRod = Config.fishingRod[i]
            break
        end
    end

    -- Check if a fishing rod was found
    if fishingRod then
        -- Remove the chosen fishing rod from the player's inventory
        RemoveItem(source, fishingRod.itemName, 1)
        TriggerClientEvent('wasabi_fishing:interupt', source)
    else
        -- Handle the case when the chosen item is not found
        print('Chosen fishing rod not found.')
    end
end)
...
RegisterNetEvent('wasabi_fishing:tryFish', function(data)
    ...
    for i = 1, #Config.fishingRod do
        xPole = HasItem(source, Config.fishingRod[i].itemName)
        xBait = HasItem(source, Config.bait.itemName)
        ...
        if xPole > 0 and xBait > 0 then
            ...
            return  -- Exit the loop since fishing was successful
        elseif xPole > 0 and xBait < 1 then
            ...
            return  -- Exit the loop since there is no bait
        elseif xPole < 1 then
            KickPlayer(source, Strings.kicked)
            return  -- Exit the loop since the player doesn't have a fishing rod
        end
    end
end)
...
for i = 1, #Config.fishingRod do
    local fishingRodItem = Config.fishingRod[i].itemName

    RegisterUsableItem(fishingRodItem, function(source)
        TriggerClientEvent('wasabi_fishing:startFishing', source, fishingRodItem)
    end)
end
