...
function GetModelFromItemName(itemName)
    if itemName == 'fishingrod' then
        return GetHashKey('prop_fishing_rod_01')
    elseif itemName == 'jewelfishingrod' then
        return GetHashKey('jewelfishingrod')
    end
    -- Add more conditions for other fishing rod models if needed
end
        ...
        if water then
            if not fishing then
                ...
                for i = 1, #Config.fishingRod do
                    local fishingRodItem = Config.fishingRod[i]
                    local hasRod, rodModel = lib.callback.await('wasabi_fishing:checkRod', 100, fishingRodItem.itemName, fishingRodItem.model)

                    if hasRod then
                        local model = rodModel
                        ...
                        while fishing do
                            ...
                            if IsDisabledControlJustReleased(0, 24) then
                                ...
                                if lib.skillCheck(fishData.difficulty) then
                                    ...
                                    ClearPedTasks(cache.ped)
                                    TriggerEvent('wasabi_fishing:notify', Strings.fish_escape, Strings.fish_escape_desc, 'error')
                                    TaskPlayAnim(cache.ped, 'amb@world_human_stand_fishing@idle_a', 'idle_c', 1.0, -1.0, 1.0, 11, 0, 0, 0, 0)
                                end
                            end
                ...
                if not fishing then
                    TriggerEvent('wasabi_fishing:notify', Strings.no_rod_equipped, Strings.no_rod_equipped_desc, 'error')
                end
            else
                TriggerEvent('wasabi_fishing:notify', Strings.already_fishing, Strings.already_fishing_desc, 'error')
            end
        else
            TriggerEvent('wasabi_fishing:notify', Strings.not_near_water, Strings.not_near_water_desc, 'error')
        end
    else
